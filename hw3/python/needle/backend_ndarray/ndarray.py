...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        if prod(self._shape) != prod(new_shape):
            raise ValueError("shape in total should be same ")
        
        if not self.is_compact():
            raise ValueError("not compact array")
    ...
    def broadcast_to(self, new_shape):
        ...
        new_strides = np.array(self._strides)
        ...
        for i in range(len(self._shape)):
            if self._shape[i] != new_shape[i]:
                if self._shape[i] != 1:
                    raise ValueError("self shape not eqausl to 1")
                else:
                    new_strides[i] = 0
    ...
    def __getitem__(self, idxs):
        ...
        '''
        new_shape = []
        for cur_slice in idxs:
            start, stop, step = cur_slice.start, cur_slice.stop, cur_slice.step
            new_shape.append(math.ceil((stop-start)/step))
        

        offset = []
        for stride, cur_slice in zip(self._strides, idxs):
            offset.append(cur_slice.start * stride)
        new_offset = sum(np.array(offset))

        new_strides = []
        for stride, cur_slice in zip(self._strides, idxs):
            new_strides.append(stride * cur_slice.step)
