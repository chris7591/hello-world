...
def test_log_forward():
    np.testing.assert_allclose(
        ndl.log(ndl.Tensor([[1.0, np.e]])).numpy(), 
        np.array([[0.0, 1.0]]), 
        rtol=1e-5, 
        atol=1e-5
    )
...
def test_exp_forward():
    np.testing.assert_allclose(
        ndl.exp(ndl.Tensor([[0.0, 1.0]])).numpy(), 
        np.array([[1.0, np.e]]), 
        rtol=1e-5, 
        atol=1e-5
    )
...
def test_relu_forward():
    np.testing.assert_allclose(
        ndl.relu(ndl.Tensor([[1.0, -2.0, 0.0]])).numpy(), 
...
def test_negate_backward():
    gradient_check(ndl.negate, ndl.Tensor(np.random.randn(3, 5)))
    gradient_check(ndl.negate, ndl.Tensor(np.random.randn(2, 4, 6)))

def test_exp_backward():
    gradient_check(ndl.exp, ndl.Tensor(np.random.randn(3, 5)))
    gradient_check(ndl.exp, ndl.Tensor(np.random.randn(2, 4, 6)))
...
def test_topo_sort():
    ...
    # Test case 4 - Simple linear chain
    a = ndl.Tensor(np.array([[2.0]]))
    b = 5 * a
    c = b + a
    d = c * 2
    ...
    topo_order = [x.numpy() for x in ndl.autograd.find_topo_sort([d])]

    assert len(soln) == len(topo_order)
    np.testing.assert_allclose(topo_order, soln, rtol=1e-06, atol=1e-06)

    # Test case 5 - Independent operations
    a = ndl.Tensor(np.array([[3.0]]))
    b = ndl.Tensor(np.array([[4.0]]))
    
    c = a * 2  # Independent operation
    d = b + 3  # Independent operation
    
    e = c + d  # The two operations finally converge here
    ...
    topo_order = [x.numpy() for x in ndl.autograd.find_topo_sort([e])]

    assert len(soln) == len(topo_order)
    np.testing.assert_allclose(topo_order, soln, rtol=1e-06, atol=1e-06)

    # Test case 6 - Mixed complex chain
    a = ndl.Tensor(np.array([[1.0, 2.0], [3.0, 4.0]]))
    b = ndl.Tensor(np.array([[2.0, 0.0], [1.0, 3.0]]))
    
    c = a @ b  # Matrix multiplication
    d = c + a  # Addition
    e = d * 2  # Element-wise multiplication
    
    soln = [
        np.array([[1.0, 2.0], [3.0, 4.0]]),  # a
        np.array([[2.0, 0.0], [1.0, 3.0]]),  # b
        np.array([[4.0, 6.0], [10.0, 12.0]]), # a @ b
        np.array([[5.0, 8.0], [13.0, 16.0]]), # c + a
        np.array([[10.0, 16.0], [26.0, 32.0]]) # d * 2
    ]
    ...
    # Test case 7 - Division and exponentiation
    a = ndl.Tensor(np.array([[9.0]]))
    b = ndl.Tensor(np.array([[3.0]]))
    
    c = a / b  # Division
    d = c ** 2  # Exponentiation
    e = d + a  # Final addition
    ...
    # Test case 8 - Subtraction and multiplication
    a = ndl.Tensor(np.array([[5.0]]))
    b = ndl.Tensor(np.array([[2.0]]))
    
    c = a - b  # Subtraction
    d = c * b  # Multiplication
    e = d + b  # Addition

    soln = [
        np.array([[5.0]]),  # a
        np.array([[2.0]]),  # b
        np.array([[-2.0]]),  # Negation of b
        np.array([[3.0]]),  # a - b (after negation)
        np.array([[6.0]]),  # c * b
        np.array([[8.0]])   # d + b
    ]
