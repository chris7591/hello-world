...
class Linear(Module):
    def __init__(
        ...
        self.bias = Parameter(init.kaiming_uniform(self.out_features, 1, device=device, dtype=dtype, requires_grad=True).reshape((1, self.out_features))) if bias else None
        ### END YOUR SOLUTION

    def forward(self, X: Tensor) -> Tensor:
        ### BEGIN YOUR SOLUTION
        first = X.matmul(self.weight)
        if self.bias is not None:
            first = first + self.bias.broadcast_to(first.shape)
        return first
        ### END YOUR SOLUTION
...
class Flatten(Module):
    def forward(self, X):
        ### BEGIN YOUR SOLUTION 
        x_shape = X.shape
        all_shapes = reduce(lambda x, y : x*y, x_shape)
        second_dimension = (int) (all_shapes / X.shape[0])
        return X.reshape((X.shape[0], second_dimension))
        ### END YOUR SOLUTION
...
class SoftmaxLoss(Module):
    def forward(self, logits: Tensor, y: Tensor):
        ### BEGIN YOUR SOLUTION
        first_term = ops.summation(ops.logsumexp(logits, axes=(1,)) / logits.shape[0]) 
        onehot = init.one_hot(logits.shape[1], y)
        second_term = ops.summation((onehot * logits) / logits.shape[0]) 
